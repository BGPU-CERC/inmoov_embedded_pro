...
{
    // _min = constrain(minAngle, minPWM, maxPWM);
    _minAngle = minAngle;
    _maxAngle = maxAngle;
    _min = map(minAngle, 0, 180, minPWM, maxPWM);
    _max = map(maxAngle, 0, 180, minPWM, maxPWM);
    // _max = constrain(maxAngle, minPWM, maxPWM);
    attach(pin, target);
}
...
void Smooth::setSpeed(int speed)
{
    ...
    _servoMaxSpeed = (long)speed * _max / (_maxAngle - _minAngle); // ~ перевод из градусов в секунду в тики
}
...
void Smooth::setAccel(int accel)
{
    ...
    _acceleration = (long)accel * (_max - _min) / (_maxAngle - _minAngle); // напрямую в градусах/сек/сек (перевод в тики)
}
...
void Smooth::setTargetDeg(int target)
{
    ...
    if (target >= _minAngle && target <= _maxAngle)
    {
        // target = constrain(target, _minAngle, _maxAngle);
        _servoTargetPos = map(target, _minAngle, _maxAngle, _min, _max);
    }
}
...
void Smooth::setCurrentDeg(int target)
{
    ...
    {
        // target = constrain(target, _minAngle, _maxAngle);
        _servoCurrentPos = map(target, _minAngle, _maxAngle, _min, _max);
    }
}
...
int Smooth::getCurrentDeg()
{
    ...
    return (map(round(_servoCurrentPos), _min, _max, _minAngle, _maxAngle));
}
...
int Smooth::getTargetDeg()
{
    ...
    return (map(_servoTargetPos, _min, _max, _minAngle, _maxAngle));
}
